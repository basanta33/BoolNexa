...
from .academy_route_catalog import ACADEMY_FIRST_ROUTE, ACADEMY_PATH_ROUTES, ACADEMY_TOTAL_LESSONS
    ...
    return rx.card(
        rx.vstack(
            ...
            rx.hstack(
                ...
                rx.spacer(),
                rx.link(
                    rx.button("Open path", size="1", variant="soft"),
                    href=ACADEMY_FIRST_ROUTE[number],
                    text_decoration="none",
                ),
                width="100%",
                align="center",
...
def _challenge_card() -> rx.Component:
    return rx.card(
        rx.vstack(
            ...
            rx.link(
                rx.button("Open NAND/XOR challenge", variant="soft", color_scheme="amber"),
                href="/academy/unit-2/mastery-challenge",
                text_decoration="none",
            ),
            spacing="3",
            align="stretch",
...
def _completed_path_browser() -> rx.Component:
    """Expose every implemented Academy lesson from the home page."""
    return rx.vstack(
        rx.hstack(
            rx.vstack(
                rx.badge("COMPLETE CURRICULUM", color_scheme="green"),
                rx.heading("All Academy lessons", size="6"),
                rx.text(
                    f"{ACADEMY_TOTAL_LESSONS} implemented lessons across seven learning paths.",
                    color="#64748b",
                ),
                align="start",
                spacing="2",
        ...
        *[
            rx.card(
                rx.vstack(
                    rx.hstack(
                        rx.badge(f"PATH {int(unit['number']):02d}", color_scheme="blue"),
                        rx.heading(str(unit["title"]), size="4"),
                        rx.spacer(),
                        rx.badge(
                            f"{len(ACADEMY_PATH_ROUTES[int(unit['number'])])} lessons",
                            color_scheme="green",
                            variant="soft",
                        ),
                        width="100%",
                    ...
                    rx.grid(
                        *[
                            ...
                            for index, (title, route) in enumerate(
                                ACADEMY_PATH_ROUTES[int(unit["number"])], start=1
                            )
                        ],
                        columns=rx.breakpoints(initial="1", md="2", xl="3"),
                        spacing="2",
